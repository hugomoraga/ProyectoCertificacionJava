...
//	    .antMatchers(HttpMethod.PATCH, "/obtiene/**").hasAuthority("ADMIN")
    ...
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {
    	...
    	.dataSource(dataSource) 
    	.usersByUsernameQuery("SELECT rut, password, enabled FROM (SELECT rut, password, enabled from estudiante UNION  SELECT rut, password, enabled From usuario) where rut=?");
    }
